...
public class Sonia {
	...
	private Sonia(PApplet p, int rate, int flag) {
		...
		if (p != null) { // for Processing >= 3.0
			...
			try {
				...
				System.err.println("[WARN] Unable to register "
						+ "dispose (ignore if not using Processing 3.x)");
				// e.printStackTrace();
			}
		}
	...
	static void initJsyn(int rate, int flag) {
		...
		try {
			...
			if (!SILENT)
				System.out.println("[SONIA] Input #" + inDevID + ": '"
						+ AudioDevice.getName(inDevID) + "' has " + inDevChNum
						+ " channel(s)");

			if (!SILENT)
				System.out.println("[SONIA] Output #" + outDevID + ": '"
						+ AudioDevice.getName(outDevID) + "' has " + outDevChNum
						+ " channel(s)");
...
//	private static void startSynth(int rate, int flag) {
//
//		if (synth != null && Sonia.synth.isRunning()) {
//			try {
//				synth.stop();
//			} catch (SynthException e) {
//				throw new SoniaException(e);
...
//	}
	...
	public static float currentTime() { // seconds
		return currentTicks() / tickRate();
	}
	
	public static float tickRate() {
		return (float) Synth.getTickRate();
	}
